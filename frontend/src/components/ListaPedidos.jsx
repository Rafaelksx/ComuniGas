...
export default function ListaPedidos() {
    ...
    const cambiarEstatus = async (id, accion) => {
        try {
            // "accion" puede ser 'verificar' o 'entregar'
            await axiosClient.patch(`/pedidos/${id}/${accion}`);

            // Actualizar la lista localmente para no tener que recargar la página
            setPedidos(pedidos.map(p => {
                if (p.id === id) {
                    return {
                        ...p,
                        estado_pago: accion === 'verificar' ? 'verificado' : p.estado_pago,
                        estado_fisico: accion === 'entregar' ? 'llena_recibida' : p.estado_fisico
                    };
                }
                return p;
            }));
                                    ...
                                    {isAdmin && (
                                        <td className="px-6 py-4 text-sm space-x-2">
                                            ...
                                            {pedido.estado_pago === 'verificado' && pedido.estado_fisico !== 'llena_recibida' && (
                                                <button onClick={() => cambiarEstatus(pedido.id, 'entregar')} className="bg-green-100 text-green-700 px-3 py-1 rounded hover:bg-green-200 font-semibold transition mt-1 md:mt-0">
                                                    Entregar Gas
                                                </button>
                                            )}
                                        </td>
